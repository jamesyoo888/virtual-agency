...
import { MOOD_OPTIONS, MOOD_LABELS, INDUSTRY_LABELS } from "@/lib/tags";
...
const VALID_MOODS = new Set<MoodTag>(MOOD_OPTIONS.map((o) => o.value));
const SITE_URL =
  process.env.NEXT_PUBLIC_APP_URL ?? "https://virtual-agency-murex.vercel.app";

export async function generateStaticParams() {
  return MOOD_OPTIONS.map((o) => ({ mood: o.value }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `${SITE_URL}/explore/mood/${tag}` },
    openGraph: {
...
export default async function ExploreMoodPage({
  params,
}: {
  params: Promise<{ mood: string }>;
}) {
      ...
      <main className="max-w-6xl mx-auto px-6 py-16 md:py-24">
        ...
        <section className="mt-16 pt-8 border-t border-zinc-900">
          ...
          <div className="flex flex-wrap gap-2">
            {MOOD_OPTIONS.filter((o) => o.value !== tag).map((o) => (
              <Link
                key={o.value}
                href={`/explore/mood/${o.value}`}
