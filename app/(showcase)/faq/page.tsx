...
const FAQ: QA[] = [
  {
    ...
    aText:
      "각 모델 카드에 일일 단가(₩/일)가 표시됩니다. 5일 이상 5%, 10일 이상 10%, 30일 이상 15% 자동 할인. 모델 상세 페이지의 견적 계산기에서 즉시 견적을 확인할 수 있으며, 최종 견적은 문의 후 확정됩니다.",
  },
  {
    ...
    aText:
      "독점은 계약 기간 동안 동일 산업 내 경쟁사가 같은 모델을 사용할 수 없습니다. 단가는 비독점 대비 약 3–5배 높지만 브랜드 자산화에 유리합니다. 비독점은 다른 클라이언트와 동시 운영이 가능하며 단가가 낮습니다.",
  },
  {
    q: "제작 기간은 얼마나 걸리나요?",
    a: (
    ...
    aText:
      "이미지·룩북: 평균 2~3 영업일. 영상 (5–10초): 평균 3~5 영업일. 3D 에셋·립싱크: 5~7 영업일. 모든 단계는 문의 → 브리프 → 제작 → 검토 → 납품의 5단계로 진행되며, 클라이언트 대시보드에서 실시간 진행률을 확인할 수 있습니다.",
  },
  {
    ...
    aText:
      "/match 페이지에서 광고 컨셉을 자유롭게 입력하거나 산업·장르·분위기·예산을 선택하면, 룰 기반 점수(산업 35pt, 장르 25pt, 분위기 20pt, 예산 보너스 + 인지도 가중치)로 모델이 정렬됩니다. 각 추천에는 점수 근거가 함께 표시되어 의사결정에 활용할 수 있습니다.",
  },
  {
    ...
    aText:
      "라이선스에 명시된 매체(SNS·옥외광고·인쇄·웹배너 등)와 지역, 기간 내에서 사용 가능합니다. 딥페이크, 성적 콘텐츠, 정치 캠페인, 실존 인물 사칭 등의 용도는 모두 이용약관 제4조에 따라 금지됩니다.",
  },
  {
    q: "환불 정책은 어떻게 되나요?",
    a: (
    ...
    aText:
      "제작 착수 전 해지 시 전액 환불. 제작 단계별로 진행된 비율만큼 차감 후 환불됩니다. 납품 완료 후에는 환불이 불가능합니다.",
  },
  {
    ...
    aText:
      "기본 패키지: 검토 단계에서 2회 무료 수정. 추가 수정은 회당 50,000원~150,000원 (모델·복잡도에 따라). 컨셉 변경 등 본질적 재작업은 별도 견적.",
  },
  {
    ...
    aText:
      "가능합니다. 모델 생성 단계에서 자동으로 3D 메시 (Meshy AI, GLB/FBX) 가 생성되며, 영상 제작 시 립싱크 (Sync AI) 옵션을 추가할 수 있습니다. 둘 다 별도 견적 단가가 적용됩니다.",
  },
  {
    q: "초상권·저작권 문제는 없나요?",
    a: (
    ...
    aText:
      "모든 모델은 AI 로 생성된 가상 인물로, 실존 인물의 외모를 모방하지 않도록 설계됩니다. 모델 자산의 저작권은 회사가 보유하며, 라이선스 범위 내에서 클라이언트가 사용할 수 있습니다.",
  },
  {
    ...
    aText:
      "관리자 시스템에 4단계 (호출·일·주·월) 비용 cap 이 적용되어 있어 AI 추론 비용 폭증을 사전 차단합니다. 클라이언트 측에는 영향이 없습니다.",
  },
  {
    ...
    aText:
      "가능합니다. 같은 모델·같은 라이선스 범위 안에서는 추가 컷이 신규 제작 대비 30~50% 저렴합니다. 분기 단위로 시리즈를 계속 확장하는 광고주들에게 가장 ROI 가 좋은 패턴입니다.",
  },
  {
    ...
    aText:
      "카탈로그에서 최대 4 명까지 컴페어 드로어에 담을 수 있고, /compare 에서 가격·산업·분위기·팔로워를 나란히 볼 수 있습니다. 매칭 점수 기반의 자동 정렬을 원하면 AI 매칭(/match)을 사용하세요.",
  },
  {
    ...
    aText:
      "로그인 후 /creator/onboard 에서 지원할 수 있습니다. 포트폴리오 URL·SNS 핸들·간단한 소개를 제출하면 관리자 검토 (보통 2~3 영업일) 후 안내드립니다.",
  },
  {
    ...
    aText:
      "모든 견적·라이선스 단가는 부가세 별도 표기이며, 정식 견적서는 부가세 포함 금액으로 발행됩니다. 사업자 등록증·세금계산서 발행 정보를 문의 시 함께 보내주시면 견적 단계에서 바로 반영합니다.",
  },
  {
    ...
    aText:
      "기본 라이선스는 국내 한정입니다. 글로벌 사용권은 별도 견적이며, 지역 범위(아시아·북미·EU 등) 와 기간을 명시한 견적을 따로 발행합니다.",
  },
  {
    q: "광고에 AI 사용 사실을 명시해야 하나요?",
    a: (
      <p>
        2026년 한국 광고심사기준(방심위·공정위 가이드)은 AI 생성 이미지·영상에 대한 명시 의무를 광고주에게 둡니다. 표기 위치는 매체에 따라 다르지만, 캠페인 크리에이티브 내 또는 캡션·디스크립션 첫 줄에 노출하는 방식이 표준입니다. Virtual Agency 는 견적 단계에서 적용 가능한 표기 가이드를 함께 제공합니다.{" "}
  ...
  {
    q: "트렌딩 페이지의 모델은 광고 효율도 좋은가요?",
    a: (
      <p>
        <Link href="/trending" className="underline hover:text-white">
          /trending
        </Link>{" "}
        은 30일 페이지뷰(광고주 호기심) 기준 정렬이며, 광고 효율(클릭률·전환율)과는 다른 신호입니다. 신규 캠페인이라면 트렌딩 + 1주 운영 데이터 병행, 분기 운영이라면 inquiry rate 가 평균 이상인 모델을 우선합니다. 두 신호가 동시에 높으면 캐스팅 1순위입니다.
      </p>
    ),
    aText:
      "/trending 은 30일 페이지뷰 기준 정렬이며 광고 효율(클릭률·전환율)과는 다른 신호입니다. 신규 캠페인이라면 트렌딩 + 1주 운영 데이터 병행, 분기 운영이라면 inquiry rate 가 평균 이상인 모델을 우선합니다.",
  },
  {
    q: "응답은 얼마나 빠르게 받을 수 있나요?",
    a: (
      <p>
        내부 SLA: 인콰이어리 도착 후 중앙값 4시간, p90 12시간 안에 1차 응답합니다. 24시간 초과 시 자동 follow-up 메일 + admin dashboard 의 stale 카운터에 노출되어 우선 처리됩니다. 카탈로그 hero 의 응답 SLA 스트립은 최근 7일 중앙값을 실시간 노출합니다.
      </p>
    ),
    aText:
      "내부 SLA: 인콰이어리 도착 후 중앙값 4시간, p90 12시간 안에 1차 응답합니다. 24시간 초과 시 자동 follow-up 메일 + admin dashboard 의 stale 카운터에 노출되어 우선 처리됩니다.",
  },
];
