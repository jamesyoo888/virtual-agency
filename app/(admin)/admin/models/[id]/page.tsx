  ...
  if (SUPABASE_CONFIGURED) {
    ...
    if (model) {
      const { data: f } = await supabase
        .from("model_files").select("*").eq("model_id", id)
        .order("created_at", { ascending: false }).limit(20);
      files = f ?? [];
    }
  } else {
    model = devModelStore.get(id) ?? null;
  }
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <Badge className={
              m.status === "active" ? "bg-green-500/20 text-green-400 border-green-500/30"
              : m.status === "draft" ? "bg-yellow-500/20 text-yellow-400 border-yellow-500/30"
              : "bg-zinc-500/20 text-zinc-400 border-zinc-500/30"
            }>{m.status}</Badge>
          </div>
          {ageYears !== null && <p className="text-zinc-400 text-sm mt-1">생체나이 {ageYears}세</p>}
        </div>
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <div className="col-span-1">
          {m.concept_image ? (
            <div className="aspect-[3/4] relative rounded-lg overflow-hidden bg-zinc-900">
              ...
              <img src={m.concept_image} alt={m.name} className="absolute inset-0 w-full h-full object-cover" />
            </div>
          ) : (
        ...
        <div className="col-span-2 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <Stat label="팔로워" value={(m.follower_count ?? 0).toLocaleString()} />
          ...
          {(m.industry_tags?.length ?? 0) > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {m.industry_tags.map((tag: string) => (
