...
import { MOOD_COPY_EN } from "@/lib/explore/copy";
...
const VALID_MOODS = new Set<MoodTag>(MOOD_OPTIONS_EN.map((o) => o.value));
const SITE_URL =
  process.env.NEXT_PUBLIC_APP_URL ?? "https://virtual-agency-murex.vercel.app";

export async function generateStaticParams() {
  return MOOD_OPTIONS_EN.map((o) => ({ mood: o.value }));
}
...
export async function generateMetadata({
  ...
  if (!VALID_MOODS.has(mood as MoodTag)) {
    return { title: "Explore — Virtual Agency" };
  }
  const tag = mood as MoodTag;
  const copy = MOOD_COPY_EN[tag];
  const ogImage = `${SITE_URL}/api/og?explore_mood=${encodeURIComponent(tag)}`;
  return {
    title: `${copy.heroTitle} — Virtual Agency`,
    description: copy.seoDescription,
    alternates: {
      canonical: `${SITE_URL}/en/explore/mood/${tag}`,
      languages: {
        en: `${SITE_URL}/en/explore/mood/${tag}`,
        ko: `${SITE_URL}/explore/mood/${tag}`,
      },
    },
    openGraph: {
      title: copy.heroTitle,
      description: copy.seoDescription,
      url: `${SITE_URL}/en/explore/mood/${tag}`,
      locale: "en_US",
      type: "website",
      images: [ogImage],
    },
...
async function loadModels(tag: MoodTag): Promise<Model[]> {
  if (!SUPABASE_CONFIGURED) {
    return (devModelStore.list() as Model[]).filter(
      (m) => m.status === "active" && m.mood_tags?.includes(tag)
    );
  }
...
export default async function EnExploreMoodPage({
  params,
}: {
  params: Promise<{ mood: string }>;
}) {
  const { mood } = await params;
  if (!VALID_MOODS.has(mood as MoodTag)) notFound();

  const tag = mood as MoodTag;
  const copy = MOOD_COPY_EN[tag];
  const models = await loadModels(tag);
  const related = relatedPostsForMood(tag, 3, "en");
      ...
      <main className="max-w-6xl mx-auto px-6 py-16 md:py-24">
        <header className="mb-12 max-w-3xl">
          ...
          <p className="text-xs tracking-[0.3em] text-zinc-500 uppercase mb-2">
            {MOOD_LABELS_EN[tag]} mood · Explore
          </p>
        ...
        {models.length === 0 ? (
          <div className="rounded-xl border border-dashed border-zinc-800 p-12 text-center text-sm text-zinc-500">
            No active models in this mood yet. Try{" "}
        ...
        {related.length > 0 && (
          <section className="mt-16 pt-8 border-t border-zinc-900">
            <h2 className="text-sm uppercase tracking-wider text-zinc-500 mb-4">
              {MOOD_LABELS_EN[tag]}-mood campaign insights
            </h2>
        ...
        <section className="mt-16 pt-8 border-t border-zinc-900">
          ...
          <div className="flex flex-wrap gap-2">
            {MOOD_OPTIONS_EN.filter((o) => o.value !== tag).map((o) => (
              <Link
                key={o.value}
                href={`/en/explore/mood/${o.value}`}
